...
if [[ ! -d $uscitaosm ]]
then
  echo "Non esiste la cartella $uscitaosm, fai girare lo script uniscibdtre.sh prima di questo"
  exit 1
fi
...
for infile in $BDTRE_IMG/66139*.osm.pbf
  do
  MAPNAME=$(basename $infile .osm.pbf)
  echo processing $MAPNAME
...
#divide e converte gli alberi in formato IMG
...
for infile in $BDTRE_IMG/66145*.osm.pbf
  do
  MAPNAME=$(basename $infile .osm.pbf)
  echo processing $MAPNAME
...
for infile in $BDTRE_IMG/66147*.osm.pbf
  do
  MAPNAME=$(basename $infile .osm.pbf)
  echo processing $MAPNAME
...
rm $uscitaosm/UNITO_quotati.pbf
rm $BDTRE_IMG/areas.*
rm $BDTRE_IMG/densities-out.txt
rm $BDTRE_IMG/temp*.*
rm $BDTRE_IMG/*.tmp
...
for infile in $BDTRE_IMG/66149*.osm.pbf
  do
  MAPNAME=$(basename $infile .osm.pbf)
  echo processing $MAPNAME
